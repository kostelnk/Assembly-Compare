...
tax_bac <- read_tsv("results/gtdbtk/myloasm/classify/gtdbtk.bac120.summary.tsv", show_col_types = FALSE) %>% 
  select(user_genome, classification)
  
tax_ar <- read_tsv("results/gtdbtk/myloasm/classify/gtdbtk.ar53.summary.tsv", show_col_types = FALSE) %>% 
  select(user_genome, classification)


# 2. Spojíme bakterie a archea do jedné tabulky a vyčistíme data
taxonomy_clean <- bind_rows(tax_bac, tax_ar) %>%
  ...
  # Volitelné: odstranění "Unclassified" pokud to zadání vyžaduje přísně
  filter(!is.na(phylum))
...
final_data <- headers_clean %>%
  ...
  left_join(quality_clean, by = "contig_id") %>%
  # Připojíme taxonomii z GTDBtk 
  left_join(taxonomy_clean, by = "contig_id")

# Kontrola výsledku - měli bychom vidět sloupec 'phylum'
glimpse(final_data)
```
